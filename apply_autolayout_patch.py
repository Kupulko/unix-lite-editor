#!/usr/bin/env python3
# apply_autolayout_patch.py
# Run from the project root:
#   python .\apply_autolayout_patch.py
...
UTILS = ROOT / "src" / "utils.ts"
EDITOR = ROOT / "src" / "editor" / "Editor.tsx"
...
def backup(path: Path) -> None:
    bak = path.with_name(path.name + ".autolayout-patch.bak")
...
def replace_regex_once(text: str, pattern: str, replacement: str, label: str) -> str:
    ...
    if count != 1:
        die(f"Could not patch: {label}")
    print(f"[OK] {label}")
    return new_text

for path in (APP, UTILS, EDITOR):
    if not path.exists():
        die(f"File not found: {path}. Run this script from the project root.")
...
app = replace_regex_once(
    app,
    r'function\s+commitScene\s*\(\s*next\s*:\s*Scene\s*\|\s*\(\(\s*prev\s*:\s*Scene\s*\)\s*=>\s*Scene\s*\)\s*\)\s*\{.*?\}\s*useEffect\s*\(',
    new_commit_scene,
    "App.tsx / commitScene",
)
...
utils = replace_regex_once(
    utils,
    r'export\s+function\s+applyAutoLayoutToScene\s*\(\s*scene\s*:\s*Scene\s*\)\s*:\s*Scene\s*\{.*$',
    new_utils_tail,
    "utils.ts / auto-layout engine",
)
...
editor = replace_regex_once(
    editor,
    r'import\s*\{\s*bringNodeForward,\s*bringNodeToFront,\s*clamp,\s*getTopLevelNodes,\s*sendNodeBackward,\s*sendNodeToBack,\s*\}\s*from\s*"../utils";',
    'import { bringNodeForward, bringNodeToFront, clamp, createAutoLayoutFrameFromDrop, findAutoLayoutDropCandidate, getTopLevelNodes, insertNodeIntoAutoLayoutFrame, sendNodeBackward, sendNodeToBack, } from "../utils";',
    "Editor.tsx / utils import",
)

editor = replace_regex_once(
    editor,
    r'const\s+\[contextMenu,\s*setContextMenu\]\s*=\s*useState\(null\);',
    'const [contextMenu, setContextMenu] = useState(null); const [autoLayoutPreview, setAutoLayoutPreview] = useState<ReturnType<typeof findAutoLayoutDropCandidate>>(null);',
    "Editor.tsx / auto-layout preview state",
)

new_drag_handlers = r'''onDragStart={(e: any) => { stopBubble(e); setContextMenu(null); setAutoLayoutPreview(null); }} onDragMove={(e: any) => { stopBubble(e); const t = e.target as Konva.Group; const worldX = ab.x + t.x(); const worldY = ab.y + t.y(); setAutoLayoutPreview(findAutoLayoutDropCandidate(scene, node.id, worldX, worldY)); }} onDragEnd={(e: Konva.KonvaEventObject) => { const t = e.target as Konva.Group; const worldX = ab.x + t.x(); const worldY = ab.y + t.y(); const movedScene: Scene = { ...scene, nodes: nodes.map((n) => n.id === node.id ? ({ ...n, x: worldX, y: worldY, } as SceneNode) : n ), }; const candidate = findAutoLayoutDropCandidate(movedScene, node.id, worldX, worldY) ?? autoLayoutPreview; if (candidate?.kind === "merge") { onChange(createAutoLayoutFrameFromDrop(movedScene, candidate.draggedId, candidate.targetId, candidate.direction, candidate.side)); } else if (candidate?.kind === "insert") { onChange(insertNodeIntoAutoLayoutFrame(movedScene, candidate.draggedId, candidate.frameId, candidate.insertIndex)); } else { onChange(movedScene); } setAutoLayoutPreview(null); }}'''

editor = replace_regex_once(
    editor,
    r'onDragStart=\{\(e:\s*any\)\s*=>\s*\{.*?\}\}\s*onDragMove=\{\(e:\s*any\)\s*=>\s*\{.*?\}\}\s*onDragEnd=\{\(e:\s*Konva\.KonvaEventObject(?:<DragEvent>)?\)\s*=>\s*\{.*?\}\}\s*(?=onTransformStart=)',
    new_drag_handlers + " ",
    "Editor.tsx / drag handlers",
)

preview_markup = r'''{autoLayoutPreview && ( <> <Rect x={autoLayoutPreview.previewRect.x} y={autoLayoutPreview.previewRect.y} width={autoLayoutPreview.previewRect.width} height={autoLayoutPreview.previewRect.height} stroke={ACCENT} strokeWidth={2} dash={[8, 5]} cornerRadius={12} listening={false} /> <Line points={[ autoLayoutPreview.insertLine.x1, autoLayoutPreview.insertLine.y1, autoLayoutPreview.insertLine.x2, autoLayoutPreview.insertLine.y2, ]} stroke={ACCENT} strokeWidth={3} lineCap="round" listening={false} /> </> )} <Transformer'''

editor = replace_regex_once(
    editor,
    r'<Transformer',
    preview_markup,
    "Editor.tsx / preview overlay",
)

EDITOR.write_text(editor, encoding="utf-8")

print()
print("[DONE] Auto Layout patch applied.")
print("Next:")
print("  npm run dev")
print()
print("Test:")
print("  1. Create two objects.")
print("  2. Drag one close to the other.")
print("  3. A blue dashed frame and insert line should appear.")
print("  4. Drop it. They should become one Auto Layout frame.")
print("  5. Drag a third object into that frame.")
