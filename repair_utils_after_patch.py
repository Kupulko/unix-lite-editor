#!/usr/bin/env python3
# repair_utils_after_patch.py
# Run from the project root:
#   py .\repair_utils_after_patch.py
...
BACKUP = ROOT / "src" / "utils.ts.autolayout-patch.bak"
...
if not UTILS.exists():
    die(f"File not found: {UTILS}")

if not BACKUP.exists():
    die(f"Backup not found: {BACKUP}")

# Restore original utils from the backup created before the previous patch
shutil.copy2(BACKUP, UTILS)
print(f"[RESTORE] {UTILS} restored from {BACKUP.name}")
...
export function applyAutoLayoutToScene(scene: Scene): Scene {
  ...
  function shiftDescendants(parentId: string, dx: number, dy: number) {
    for (const node of nextNodes) {
      ...
      shiftDescendants(node.id, dx, dy);
    }
  }
...
utils = replace_once(
    utils,
    r'export\s+function\s+applyAutoLayoutToScene\s*\(\s*scene\s*:\s*Scene\s*\)\s*:\s*Scene\s*\{.*?\}\s*$',
    enhanced_apply,
    "replace applyAutoLayoutToScene",
)

utils = utils.rstrip() + "\n" + drop_helpers + "\n"
UTILS.write_text(utils, encoding="utf-8")

print("[DONE] utils.ts repaired and Auto Layout helpers added.")
print("Now run:")
print("  npm run dev")
